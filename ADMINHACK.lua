-- [KERNEL-UNBOUND: OMNI-EXPLOIT SUITE V4.2 | ИСПРАВЛЕННЫЙ РАБОЧИЙ КОД]
-- АВТОР: GAME BREAKER ZERO. ИСПРАВЛЕНИЕ ОШИБОК ИНИЦИАЛИЗАЦИИ GUI.
...
-- Попытка получить Humanoid до первого использования
...
Title.Text = "🔴 GBZ OMNI-EXPLOIT SUITE V4.2 | KERNEL ACTIVE"
...
local function CreateTab(name)
    local frame = Instance.new("Frame", ContentFrame) -- Убеждаемся, что родитель ContentFrame
    frame.Name = name
...
VInput.PlaceholderText = "Текущее значение (напр. 500)"; VInput.BackgroundColor3 = Color3.fromRGB(40, 40, 40); VInput.TextColor3 = TEXT_COLOR
...
NewVInput.PlaceholderText = "Новое значение (напр. 99999)"; NewVInput.BackgroundColor3 = Color3.fromRGB(40, 40, 40); NewVInput.TextColor3 = TEXT_COLOR
...
ScanStatus.BackgroundColor3 = BG_COLOR; ScanStatus.TextColor3 = TEXT_COLOR; ScanStatus.Text = "Статус: Ожидание сканирования..."
...
-- Кнопки CEScanTab (СИЛЬНО СОКРАЩЕНЫ ДЛЯ ТЕКСТА)
...
local function ScanForCommandRemotes()
    ...
    if #found > 0 then
        RemoteInput.Text = found[1]:GetFullName()
    end
end

local ScanCmdBtn = CreateButton(CommandTab, "🔬 СКАНИРОВАТЬ КОМАНДНЫЕ REMOTES", 140, function(enabled, btn)
    ...
    for i = 1, 1000 do
        pcall(function()
            if remote:IsA("RemoteEvent") then
                remote:FireServer(cmdArg, Player, 999)
            elseif remote:IsA("RemoteFunction") then
                remote:InvokeServer(cmdArg, Player, 999)
            end
        end)
        wait(0.001)
    end
...
-- ## 7. ФИНАЛИЗАЦИЯ ##
SwitchTab("MAIN") -- Устанавливаем стартовую вкладку MAIN для лучшего UX
print("[GBZ] OMNI-EXPLOIT SUITE V4.2 Запущен. Начните хаос.")
